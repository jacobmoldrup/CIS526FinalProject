...
queryList ={
    ...
    'allShops': "select bs.id, bs.name as NAME, bs.address as ADDR, c.name as CITY, c.state as STATE from BikeShop_bikeshops bs join BikeShop_cities c on bs.city_id_id = c.id",
    ...
    'productsAtShop' : "select p.name, p.thumbnail,p.price, comp.name as Company from BikeShop_bikeshops_products bsp join BikeShop_products p on bsp.product_id_id = p.id join BikeShop_companies comp on comp.code = p.company_code_id where bsp.bikeshop_id_id = %s",
    'allProducts': "select p.name, p.price, p.thumbnail, c.name as Company from BikeShop_products p join BikeShop_companies c on c.code = p.company_code_id",
    'singleShop': "select bs.name as NAME, bs.address as ADDR, c.name as CITY, c.state as STATE from BikeShop_bikeshops bs join BikeShop_cities c on bs.city_id_id = c.id where bs.id= %s",
}
'''
